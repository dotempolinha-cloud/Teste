...
/* ═══ REPORTS ═══ */
function Reports({toast,vehicles,drivers,trips,fuel,maint,fines,vistorias=[],rotas=[]}){
  ...
  const gerarPDF=(tipo)=>{
    ...
    if(tipo==="Relatório Supremo"){
      ...
      const kmTotal=trips.filter(t=>t.kmRodado>0).reduce((a,t)=>a+t.kmRodado,0);
      corpo=`
        ...
        <h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">3. Motoristas (${drivers.length} cadastrados)</h2>
        <table><thead><tr><th>Matrícula</th><th>Nome</th><th>Cargo</th><th>Secretaria</th><th>Cat. CNH</th><th>Validade CNH</th><th>Situação</th><th>Viagens</th></tr></thead><tbody>
        ${drivers.map(d=>`<tr><td>${d.mat||"—"}</td><td><b>${d.nome}</b></td><td>${d.cargo}</td><td>${d.sec}</td><td>Cat. ${d.cnh}</td><td>${d.valCnh||"—"}</td><td>${d.sit}</td><td>${d.viagens||0}</td></tr>`).join("")}
        </tbody></table>

        <h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">4. Viagens (${trips.length} registros)</h2>
        ...
        <h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">5. Abastecimentos (${fuel.length} registros)</h2>
        <table><thead><tr><th>Código</th><th>Placa</th><th>Motorista</th><th>Data</th><th>Tipo</th><th>Litros</th><th>Total R$</th></tr></thead><tbody>
        ${fuel.map(f=>`<tr><td>${f.id}</td><td>${f.placa}</td><td>${f.mot||"—"}</td><td>${f.data}</td><td>${f.tipo}</td><td>${f.litros.toFixed(1)} L</td><td><b>R$ ${f.total.toFixed(2)}</b></td></tr>`).join("")}
        </tbody></table><p class="total">Total: ${fuel.reduce((a,x)=>a+x.litros,0).toFixed(1)} litros — R$ ${totalC.toFixed(2)}</p>

        <h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">6. Manutenções (${maint.length} ordens)</h2>
        <table><thead><tr><th>OS</th><th>Veículo</th><th>Tipo</th><th>Descrição</th><th>Oficina</th><th>Custo</th><th>Status</th></tr></thead><tbody>
        ${maint.map(m=>`<tr><td>${m.id}</td><td>${m.placa}</td><td>${m.tipo}</td><td>${m.desc}</td><td>${m.oficina||"—"}</td><td><b>R$ ${m.custo.toFixed(2)}</b></td><td>${m.status}</td></tr>`).join("")}
        </tbody></table><p class="total">Total: R$ ${totalM.toFixed(2)}</p>

        <h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">7. Multas (${fines.length} registros)</h2>
        <table><thead><tr><th>Código</th><th>Veículo</th><th>Data</th><th>Infração</th><th>Valor</th><th>Status</th></tr></thead><tbody>
        ${fines.map(m=>`<tr><td>${m.id}</td><td>${m.placa}</td><td>${m.data}</td><td>${m.inf}</td><td><b>R$ ${m.valor.toFixed(2)}</b></td><td>${m.status}</td></tr>`).join("")}
        </tbody></table><p class="total">Multas pendentes: R$ ${multasPend.toFixed(2)}</p>

        ${vistorias.length>0?`<h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">8. Vistorias (${vistorias.length} registros)</h2>
        ...
        ${vistorias.map(v=>`<tr><td>${v.id}</td><td>${v.placa}</td><td>${v.mot}</td><td>${v.data}</td><td>${v.estadoCons||"—"}</td><td>${v.ok}/${v.total}</td><td>${v.res}</td></tr>`).join("")}
        </tbody></table>`:""}

        ${rotas.length>0?`<h2 style="color:#0c1a47;border-bottom:3px solid #1d4ed8;padding-bottom:8px;margin:20px 0 12px">9. Rotas Fixas (${rotas.length} cadastradas)</h2>
        <table><thead><tr><th>Nome</th><th>Veículo</th><th>Motorista</th><th>Destino</th><th>Dias</th><th>Status</th></tr></thead><tbody>
        ${rotas.map(r=>`<tr><td><b>${r.nome}</b></td><td>${r.placa||"—"}</td><td>${r.mot}</td><td>${r.dest}</td><td>${(r.diasSemana||[]).map(d=>["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"][d]).join(", ")||"—"}</td><td>${r.ativo?"Ativa":"Inativa"}</td></tr>`).join("")}
        </tbody></table>`:""}
        ...
        <div style="margin-top:24px;padding:14px;background:#f8fafc;border:1px solid #e2e8f0;font-size:11px;color:#64748b;text-align:center">
          Relatório Supremo gerado em ${data} às ${hora} — SGA Frota Municipal — Prefeitura de Upanema — RN<br/>
          Total geral de gastos: <b style="color:#0c1a47">R$ ${(totalC+totalM).toLocaleString("pt-BR",{minimumFractionDigits:2})}</b> · 
          Frota: <b>${vehicles.length} veículos</b> · Motoristas: <b>${drivers.length}</b> · Viagens: <b>${trips.length}</b>
        </div>
      `;
    } else
  ...
  const rpts=[
    {t:"Relatório Geral",d:"Relatório completo: frota, motoristas, viagens, combustível, manutenções, multas, vistorias e rotas — tudo em um só documento",I:Shield},
...
/* ═══ ALERTAS AUTOMÁTICOS — roda ao carregar e quando veículos/motoristas mudam ═══ */
  ...
  const pages={
    ...
    reports:<Reports toast={toast} vehicles={vehicles} drivers={drivers} trips={trips} fuel={fuel} maint={maint} fines={fines} vistorias={vistorias} rotas={rotas}/>,
